...
class TransactionRepository implements TransactionRepositoryInterface
{
    ...
    {
        $data['code'] = $this->genereteTransactionCode();
        $data['payment_status'] = 'pending';
        $data['transaction_date'] = now();
        $total = $this->calculateTotalAmount($room->price_per_month, $data['duration']);
        $data['total_amount'] = $this->caculatePaymentAmount($total, $data['payment_method']);
        return $data;
    }

    private function genereteTransactionCode()
    {
        return 'NGKAKB' . rand(100000, 999999);
    }
    ...
    private function caculatePaymentAmount($total, $paymentMethod)
    {
